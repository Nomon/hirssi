...
suite('IRCManager.createIrcService', function() {
  var pub = redis.createClient();
  var sub = redis.createClient();
  var m = new IRCManager(pub, sub);
  test('createIrcService should not return a error', function() {
    m.createIrcService({userId: 123, username: 'orktess', network: {id: 1, servers:[
          {host: "localhost", port: "6667"}
        ]}, channels: ["#test"]}, function (err, s) {
          assert.equals(err, null);
        });
  });

  test('createIrcService should return a error', function() {
    setTimeout(function () {
      m.createIrcService({userId: 123, username: 'orktess', network: {id: 1, servers:[
          {host: "localhost", port: "6667"}
        ]}, channels: ["#test"]}, function (err, s) {
          assert.equals(err.message, "IRCService already open");
      });
    }, 2000);
  });
});
