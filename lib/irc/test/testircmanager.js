    ...
    m.createIrcService({userId: userId, username: username, network: {id: 1, servers:[
          ...
          m.createIrcService({userId: userId2, username: username2, network: {id: 1, servers:[
                ...
                m.getIrcClient({userId: userId2, username: username2, network: {id: 1}}, function (err, c) {
                      ...
                      c.on('join', function (channel) {
                        c.getEvents(channel, 0, -1, function (err, messages) {
                          console.log("events from " + channel, messages);
                        });
                      });
                      c.on('message', function (from, to, message) {
                        ...
                        c.getEvents(from, 0, -1, function (err, messages) {
                          ...
                          console.log("events with " + from, messages);
                          ...
                          c.getChannels(function (err, channels) {
                            console.log("received channels", channels);
                            var channelName;
                            for(channelName in channels) {
                              c.getEvents(channelName, 0, -1, function (err, messages) {
                                console.log("events from " + channelName, messages);
                              });
                            }

                          });

                          c.getMe(function (err, me) {
                            console.log("received me", me);
                          });

                          c.whois(username2, function (err, whoisData) {
                            console.log("received whois", whoisData);
                          });
