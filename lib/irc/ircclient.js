...
var IRCClient = function (options) {
  ...
  this.sub.on('message', function (channel, message) {
    if (channel === self.channel) {
      ...
      var i, length = self.emitters.length;
      for (i = 0; i < length; i++) {
        self.emitters[i].emit.apply(self, parts);
      }
    }
  });
...
IRCClient.prototype.addEmitter = function (emitter) {
...
IRCClient.prototype.removeEmitter = function (emitter) {
  var indx = this.emitters.indexOf(emitter);
  if (indx == -1)
    return;

  this.emitters.splice(indx, 1);
};
...
IRCClient.prototype.getChannels = function (callback) {
  var self = this;
  if (typeof callback === 'function') {
    this.once('channels', function () {
        var args = Array.prototype.slice.call(arguments, 0);
        args.unshift(null);
        callback.apply(this, args);
    });
...
IRCClient.prototype.getMe = function (callback) {
  var self = this;
  if (typeof callback === 'function') {
