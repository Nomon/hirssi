...
IRCService.prototype._initListeners = function () {
  ...
  this.on('say', function (target, message) {
    ...
    self.pub.zadd('ircmessages:' + target + ":" + self.userId, (new Date()).getTime(), JSON.stringify({f: self.username, t: target, m: message}));
...
IRCService.prototype._initIrcClientListeners = function () {
  ...
  this.ircClient.on('message', function (from, to, message) {
   ...
   self.pub.zadd('ircmessages:' + arch + ":" + self.userId, (new Date()).getTime(), JSON.stringify({f: from, t: to, m: message}));
  });
