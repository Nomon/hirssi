...
var IRCService = function (options) {
  ...
  this.nick = options.nick || options.username;
  this.realname = options.realname || options.username;
  ...
  this.sub.subscribe("ircservices");
...
IRCService.prototype._initListeners = function () {
  var self = this;
  this.on('status_check', function () {
    self.setStatus(self.status);
  });

  this.on('say', function (target, message) {
    self.ircClient.say(target, message);
    self._recordEvent(target, {et: "msg", t: target, f: self.ircClient.nick, m: message});
    self.cmd(['message', self.ircClient.nick, target, message]);
  });
  ...
  this.on('whois', function (target) {
    self.ircClient.whois(target, function () {
    });
  });
  ...
  this.on('me', function () {
    self.cmd(['me', {
      nick: self.ircClient.nick,
      username: self.ircClient.opt.userName,
      realname: self.ircClient.opt.realName,
      host: self.ircClient.opt.server,
      port: self.ircClient.opt.port
    }]);
  });
...
IRCService.prototype.open = function (callback) {
  var self = this;
  this.ircClient = new irc.Client(this.network.servers[0].host, this.nick, {
...
IRCService.prototype._initIrcClientListeners = function () {
  ...
  this.ircClient.on('mode', function (channel, mode, target) {
    self._recordEvent(channel, {et: "mde", m: mode, t: target});
  });
  ...
  this.ircClient.emit = function (type) {
    ...
    if (type != "newListener" && type != "raw" && type != "removeListener") {
      ...
      var msg = arguments[1];
      if (msg.command == "MODE") {
        var args2 = ["mode"].concat(msg.args);
        self.ircClient.emit.apply(this.ircClient, args2);
      }
    }
