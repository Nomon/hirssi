...
var getIrcConfiguration = function(network, cb) {
  ...
  network.servers(function(err, servers) {
    ...
    network.channels(function(err, channels) {
      if (err) {
        return cb(err);
      }
      for (i in channels) {
      ...
      return cb(err, {servers: srvrs, channels: chans});
    });
  });
}
      ...
      user.networks(function(err, networks) {
        networks.forEach(function(network) {
          ...
          getIrcConfiguration(network, function(err, config) {
            ...
            networkConfiguration.network.servers = config.servers;
            networkConfiguration.channels = config.channels;
            console.dir(networkConfiguration);
            self.manager.createIrcService(networkConfiguration, function(err) {
              if (err) {
                console.log(err);
              }
              /**
...
exports.getIrcConfiguration = getIrcConfiguration;